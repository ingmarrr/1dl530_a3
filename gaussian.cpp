...
void gaussian_v1_par(int n, long long** A, long* b, long* x, int threads, std::string runtime)
{
	int row = 0, col = 0;
...
void gaussian_v2_par(int n, long long** A, long* b, long* x, int threads, std::string runtime)
{
	int row = 0, col = 0;
...
int main(int argc, char* argv[])
{
	...
	std::cout << "[info] dimension (number of variables): " << dim << std::endl;
	...
	for (size_t i = 0; i < schedules.size(); ++i) 
	{
		...
		x = new long[dim];
		gaussian_v1_par(dim, A, b, x, nb_threads, schedule_names[i]);
		...
		x = new long[dim];
		gaussian_v2_par(dim, A, b, x, nb_threads, schedule_names[i]);
